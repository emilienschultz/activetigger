...
def app_navigation():
    ...
    if st.session_state.state:
        if (
            st.session_state.user in st.session_state.state["bertmodels"]["training"]
        ) or st.session_state.user in st.session_state.state["simplemodel"]["training"]:
            st.session_state.bert_training = True
            col1, col2 = st.columns((3, 1))
            with col1:
                st.html(
                    f"<div style='background-color: #ffcc00; padding: 10px;'>Computing (training / predicting). Wait the process to end before launching another one or stop it.</div>"
                )
            with col2:
        ...
        if not len(c) == 0:
            st.html(
                f"<div style='background-color: #ffcc00; padding: 10px;'>Processes currently running: {c}</div>"
            )

    # creating the menu
    options = ["Projects", "Annotate", "Statistics", "Train", "Test", "Export", "Doc"]
    ...
    st.session_state["page"] = option_menu(
        f"pyActiveTigger {__version__} - Current user : {st.session_state.user} ({st.session_state.status}) - Current project : {st.session_state.current_project}",
        options,
...
def display_projects():
    ...
    if st.session_state.current_project == "create_new":
        ...
        if file:
            ...
            n_train = st.number_input(
                "N train", min_value=100, max_value=len(df), key="n_train"
            )
            n_test = st.number_input(
                "N test (0 if no test set)",
                min_value=0,
                max_value=len(df),
                key="n_test",
            )
    ...
    if st.session_state.current_project and (
        ...
        if not st.session_state.state:
            st.session_state.state = _get_state()
        st.markdown(
            f"<hr>Current project loaded : {st.session_state.current_project} <br>",
...
def display_schemes():
    ...
    col1, col2 = st.columns(2)
    with col1:
        new_scheme = st.text_input(
            label="New scheme", placeholder="New scheme name", label_visibility="hidden"
        )
    with col2:
        st.markdown("<br>", unsafe_allow_html=True)
...
def display_features():
    ...
    feature = st.selectbox(
        label="Available", options=st.session_state.state["features"]["available"]
    )
    ...
    add_feature = st.selectbox(
        label="Add", options=list(st.session_state.state["features"]["options"].keys())
    )

    params = ""
    if add_feature in st.session_state.state["features"]["options"]:
        params = json.dumps(
            st.session_state.state["features"]["options"][add_feature], indent=2
        )
        params = st.text_area(
            label="Parameters", value=params, label_visibility="hidden"
        )
    ...
    st.subheader("Regex")
    st.dataframe(
        pd.Series(st.session_state.state["features"]["infos"], name="Matched")
    )  # display existing regex
    regex = st.text_input(
        label="Regex", placeholder="Write your regex", label_visibility="hidden"
    )
...
def display_annotate():
    ...
    with col3:
        ...
        if st.session_state.selection == "maxprob":
            tag_options = st.session_state.state["schemes"]["available"][
    ...
    if st.session_state.current_element:
        ...
        if (
            len(st.session_state.current_element["text"])
            < st.session_state.current_element["limit"]
        ):
            text_in = st.session_state.current_element["text"]
            text_out = ""
        else:
            text_in = st.session_state.current_element["text"][
                0 : st.session_state.current_element["limit"]
            ]
            text_out = st.session_state.current_element["text"][
                st.session_state.current_element["limit"] :
            ]
...
def display_projection():
    ...
    st.text_area(
        label="Parameters",
        value=json.dumps(
            st.session_state.state["projections"]["available"]["umap"], indent=2
        ),
    ...
    if ("projection_data" in st.session_state) and (
        type(st.session_state.projection_data) == str
    ):
        ...
        if r["status"] == "computed":
            ...
            if not "projection_visualization" in st.session_state:
                ...
                st.session_state.projection_visualization.update_layout(
                    {"uirevision": "foo"}, overwrite=True
                )

    if "projection_visualization" in st.session_state:
        st.plotly_chart(
            st.session_state.projection_visualization, use_container_width=True
        )
...
def display_manage_tags():
    ...
    col1, col2 = st.columns(2)
    options_labels = []
    options_labels = st.session_state.state["schemes"]["available"][
        st.session_state.current_scheme
    ]
    with col1:
        new_label = st.text_input(
            label="New label", placeholder="New label name", label_visibility="hidden"
        )
    with col2:
        st.markdown("<br>", unsafe_allow_html=True)
...
def _display_labels():
    ...
    Display labels
    """
    labels = st.session_state.state["schemes"]["available"][
        st.session_state.current_scheme
    ]
    if len(labels) == 0:
...
def display_data():
    ...
    st.session_state.data_df["label"] = (
        st.session_state.data_df["label"].astype("category")
    ).cat.add_categories(
        [l for l in labels if not l in st.session_state.data_df["label"].unique()]
    )
    modified_table = st.data_editor(
        st.session_state.data_df[["label", "text"]], disabled=["text"]
    )
...
def display_zeroshot():
    ...
    with col2:
        st.session_state.api_token = st.text_input(
            "Token", value=st.session_state.api_token
        )
    ...
    if not "codebook" in st.session_state:
        st.session_state.codebook = json.dumps(
            {i: "Write the rule" for i in labels}, indent=2
        )
    st.session_state.codebook = st.text_area(
        "Codebook", value=st.session_state.codebook
    )
    prompt = None
    ...
    if pd.notna(st.session_state.state["zeroshot"]["data"]):
        if st.session_state.state["zeroshot"]["data"] == "computing":
            ...
            df = pd.read_json(
                st.session_state.state["zeroshot"]["data"], dtype={"index": str}
            ).set_index("index")
            df["zero_shot"] = (df["zero_shot"].astype("category")).cat.add_categories(
                [l for l in labels if not l in df["zero_shot"].unique()]
            )
            st.session_state.df_zeroshot = st.data_editor(
                df[["zero_shot", "text"]], disabled=["text"]
            )
...
def display_simplemodels():
    ...
    if (
        st.session_state.user in st.session_state.state["simplemodel"]["available"]
    ) and (
        st.session_state.current_scheme
        in st.session_state.state["simplemodel"]["available"][st.session_state.user]
    ):
        status = "Trained"
        current_model = st.session_state.state["simplemodel"]["available"][
    ...
    if (
        ...
        st.session_state.current_scheme
        in st.session_state.state["simplemodel"]["training"][st.session_state.user]
    ):
        status = "Computing"
    ...
    st.markdown(
        f"<b>{status}</b> - Current model: {current_model} <br> {statistics}",
        unsafe_allow_html=True,
    )
    ...
    st.selectbox(label="Model", key="sm_model", options=available_models, index=index)
    # display current params if same model else default params
    params = json.dumps(
        st.session_state.state["simplemodel"]["options"][st.session_state.sm_model],
        indent=2,
    )
    sm = _get_simplemodel()
    ...
    st.slider(
        label="Training frequency", min_value=5, max_value=100, step=1, key="sm_freq"
    )
    if st.button("Train"):
...
def display_bertmodels():
    ...
    with st.expander("Existing models"):
        ...
        if (
            st.session_state.current_scheme
            in st.session_state.state["bertmodels"]["available"]
        ):
            available_bert = list(
                st.session_state.state["bertmodels"]["available"][
                    st.session_state.current_scheme
                ].keys()
            )
        st.selectbox(
    ...
    with st.expander("Training model"):
        ...
        st.text_area(
            ...
            key="bm_params",
            value=json.dumps(
                st.session_state.state["bertmodels"]["base_parameters"], indent=2
            ),
        )
...
def display_export():
    ...
    with st.expander("Bert"):
        ...
        if (
            ...
            available = st.session_state.state["bertmodels"]["available"][
                st.session_state.current_scheme
            ]
...
def display_test():
    ...
    model_to_test = st.selectbox(
        label="Trained models", options=available_bert, key="bm_trained_test"
    )
...
def display_reconciliate():
    ...
    df["Reconciliate"] = (df["Reconciliate"].astype("category")).cat.add_categories(
        labels
    )
...
def _get(
    route: str, params: dict | None = None, data: dict | None = None, binary=False
) -> dict:
    """
    Get from API
    """
    url = URL_SERVER + route
    r = rq.get(
        url, params=params, data=data, headers=st.session_state.header, verify=False
    )
    if r.status_code == rq.codes.ok:
        if binary:
            return r.content
...
def _create_user(username: str, password: str, status: str):
    ...
    Create user
    """
    params = {"username_to_create": username, "password": password, "status": status}
    r = _post(route="/users/create", params=params)
    if (r is not None) and ("error" in r):
        st.write(r["error"])
        return False
    return r
...
def _delete_user(username: str):
    ...
    r = _post(route="/users/delete", params=params)
    if (r is not None) and ("error" in r):
        st.write(r["error"])
        return False
    return r
...
def _create_scheme(s: str):
    ...
    r = _post("/schemes/add", params=params, json_data=data)
    return r
...
def _create_label(label: str):
    ...
    r = _post("/schemes/label/add", params=params)
    return r
...
def _delete_label(label: str):
    ...
    r = _post("/schemes/label/delete", params=params)
    return r
...
def _get_next_element() -> bool:
    ...
    r = _post(route="/elements/next", params=params, json_data=data)
...
def _compute_projection():
    ...
    r = _post("/elements/projection/compute", params=params, json_data=data)
    if r["status"] == "waiting":
...
def _get_projection_data():
    ...
    r = _get("/elements/projection/current", params=params)
    return r
...
def _train_simplemodel():
    ...
    """
    if (st.session_state.sm_features is None) or (
    ...
    r = _post("/models/simplemodel", params=params, json_data=data)
    if (r is not None) and ("error" in r):
        st.write(r["error"])
        return False
...
def _bert_prediction():
    ...
    r = _post("/models/bert/predict", params=params)
    if (r is not None) and ("error" in r):
        st.write(r["error"])
    return True
...
def _delete_bert():
    ...
    r = _post("/models/bert/delete", params=params)
    return r
...
def _save_bert():
    ...
    r = _post("/models/bert/rename", params=params)
    return r
...
def _start_bertmodel():
    ...
    r = _post("/models/bert/train", params=params, json_data=data)
    time.sleep(2)
    st.session_state.bert_training = True
    return True
...
def _export_data():
    ...
    r = _get("/export/data", params=params, binary=True)
    return r
...
def _export_features():
    ...
    r = _get("/export/features", params=params, binary=True)
    return r
...
def _export_predictions():
    ...
    r = _get("/export/prediction", params=params, binary=True)
    return r
...
def _get_simplemodel():
    if _is_simplemodel():
        return st.session_state.state["simplemodel"]["available"][
            st.session_state.user
        ][st.session_state.current_scheme]
    return None
...
def _compute_test(model_name, scheme):
    ...
    r = _post("/models/bert/test", params=params)
    if (r is not None) and ("error" in r):
        st.write(r["error"])
        return False
    return True
...
def _post_reconciliation_table(new_labels: dict, users: list):
    ...
    for i, j in new_labels.items():
        ...
        r = _post("/elements/reconciliate", params=params)
        if (r is not None) and ("error" in r):
            st.write(r["error"])
            return False
        st.write(i)
...
def _rename_label(former_label: str, new_label: str):
    ...
    r = _post("/schemes/label/rename", params=params)
    if (r is not None) and ("error" in r):
        st.write(r["error"])
        return False
    return True
