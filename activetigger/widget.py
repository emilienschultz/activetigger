...
class Widget:
    ...
    def _get(
        ...
        """
        url = self.URL_SERVER + route
        r = rq.get(url, params=params, data=data, headers=self.headers, verify=False)
    ...
    def start(self) -> None:
        ...
        try:
            ...
            print(
                f"Failed to connect to the server. Please check if the server is available at {self.URL_SERVER}"
            )
            return None
        ...
        # Users
        self.existing_users = widgets.Dropdown(
            description="User:", options=existing["users"], layout={"width": "200px"}
        )
        ...
        self.connect_user.on_click(
            lambda x: self._connect_user(
                user=self.existing_users.value, password=self.password.value
            )
        )
        ...
        self.output = widgets.VBox(
            [
                widgets.HBox(
                    [img_at, self.existing_users, self.password, self.connect_user]
                ),
                self.current_user,
    ...
    def _delete_project(self, project_name: str) -> dict:
        ...
        """
        params = {"project_name": project_name, "user": self.user}
        r = self._post(route="/projects/delete", params=params)
        self.start()
        return r
    ...
    def _create_new_project(self):
        ...
        project_name = widgets.Text(
            disabled=False, description="Name:", layout={"width": "200px"}
        )
        ...
        dic_langage = {"French": "fr", "English": "en", "Spanish": "es"}

        # select columns
        column_text = widgets.Dropdown(
            options=[], description="Text:", layout={"width": "200px"}, disabled=False
        )

        column_id = widgets.Dropdown(
            options=[], description="Id:", layout={"width": "200px"}, disabled=False
        )
        ...
        validate = widgets.Button(description="Create", layout=layout)
        ...
        def create_project(b):
            ...
            files = {"file": (file.value[0]["name"], BytesIO(file.value[0]["content"]))}
            r = self._post(route="/projects/new", files=files, data=data)
            # if project exit
            print(r)
            if r["status"] == "error":
                print(r["message"])
        ...
        def load_file(b):
            ...
            if len(self.output.children) == 1:
                self.output.children = list(self.output.children) + [
                    separate,
                    language,
                    info,
                    column_id,
    ...
    def _display_next(self) -> bool:
        ...
        r = self._get(route="/elements/next", params=params)
    ...
    def _display_buttons_labels(self) -> bool:
        ...
        def send_tag(v):
            data = {
                ...
                "tag": v.description,
                "user": self.user,
                "selection": self.current_element[
                    "selection"
                ],  # mode of selection of the element
            }
            ...
            if self.is_simplemodel() and (
                len(self.history) % self.simplemodel_autotrain.value == 0
            ):
                sm = self.state["simplemodel"]["available"][self.user][
                    self.select_scheme.value
                ]
                self.create_simplemodel(
                    self.select_scheme.value,
                    model=sm["name"],
    ...
    def is_simplemodel(self) -> bool:
        if self.user in self.state["simplemodel"]["available"]:
            if (
                self.select_scheme.value
                in self.state["simplemodel"]["available"][self.user]
            ):
                return True
        return False
    ...
    def update_tab_bertmodels(self, state=True):
        ...
        if self.select_scheme.value in self.state["bertmodels"]["available"]:
            self.available_bert.options = self.state["bertmodels"]["available"][
                self.select_scheme.value
            ].keys()
        self.new_bert_params.value = json.dumps(
            self.state["bertmodels"]["base_parameters"], indent=2
        )
    ...
    def update_tab_features(self, state=True):
        ...
        self.features_params.value = json.dumps(
            self.state["features"]["options"][self.add_features.value]
        )
    ...
    def update_tab_annotations(self, state=True):
        ...
        if self.is_simplemodel():
            ...
            self._mode_label.options = self.state["schemes"]["available"][
                self.select_scheme.value
            ]
        # projection
        self.projection_method.options = list(
            self.state["projections"]["available"].keys()
        )
        self.projection_params.value = json.dumps(
            self.state["projections"]["available"]["umap"], indent=2
        )
    ...
    def update_tab_schemes(self, state=True):
        ...
        self.select_label.options = self.state["schemes"]["available"][
            self.select_scheme.value
        ]
    ...
    def update_tab_simplemodel(self, state=True):
        ...
        self.select_simplemodel.options = list(
            self.state["simplemodel"]["options"].keys()
        )
        ...
        if (self.user in self.state["simplemodel"]["available"]) and (
            self.select_scheme.value
            in self.state["simplemodel"]["available"][self.user]
        ):
            current_model = self.state["simplemodel"]["available"][self.user][
                self.select_scheme.value
            ]
            name = current_model["name"]
            statistics = f"F1: {round(current_model['statistics']['weighted_f1'],2)} - accuracy: {round(current_model['statistics']['accuracy'],2)}"
            self.simplemodel_params.value = json.dumps(
                current_model["params"], indent=2
            )
            self.select_simplemodel.value = name

        # if there is a model under training
        if (self.user in self.state["simplemodel"]["training"]) and (
            self.select_scheme.value in self.state["simplemodel"]["training"][self.user]
        ):
            status = "Computing"
    ...
    def update_tab_data(self, state=True):
        ...
        for i, j in df.iterrows():
            ...
            menu = widgets.Dropdown(
                options=options, value=j["labels"], layout={"width": "100px"}
            )
            menu.layout.id = i
            buttons.append(
                widgets.HBox(
                    [
                        widgets.HTML(
                            value=f"<hr>{i} - <small>{j['text']}</small>",
                            layout={"width": "500px"},
                        ),
    ...
    def create_scheme(self, s):
        ...
        r = self._post("/schemes/add", params=params, json_data=data)
    ...
    def create_label(self, text_field):
        ...
        r = self._post("/schemes/label/add", params=params)
        self.update_tab_schemes()
        text_field.value = ""
        return r
    ...
    def delete_scheme(self, s):
        ...
        r = self._post("/schemes/delete", params=params, json_data=data)
        self.update_tab_schemes()
        return r
    ...
    def delete_label(self, label: str):
        ...
        r = self._post("/schemes/label/delete", params=params)
        self.update_tab_schemes()
        return r
    ...
    def _delete_bert(self, bert_name):
        ...
        r = self._post("/models/bert/delete", params=params)
        self.update_tab_bertmodels()
        return r

    def create_simplemodel(
        self, scheme: str, model: str, parameters: str, features: list
    ):
        """
        ...
        r = self._post("/models/simplemodel", params=params, json_data=data)
        if "error" in r:
    ...
    def create_bertmodel(self):
        ...
        r = self._post("/models/bert/train", params=params, json_data=data)
        time.sleep(2)
    ...
    def _start_bert_prediction(self, model_name: str):
        ...
        r = self._post("/models/bert/predict", params=params)
        if "error" in r:
            print(r)
        print(r)
    ...
    def stop_bertmodel(self):
        ...
        """
        self.compute_new_bert.disabled = True
        params = {"project_name": self.project_name, "user": self.user}
        r = self._post("/models/bert/stop", params=params)
        time.sleep(2)
    ...
    def save_bert(self, former_name, new_name):
        ...
        r = self._post("/models/bert/rename", params=params)
        self.update_tab_bertmodels()
        return r
    ...
    def export_data(self, format):
        ...
        r = self._get("/export/data", params=params, is_json=False)
    ...
    def export_features(self, features: list, format: str):
        ...
        r = self._get("/export/features", params=params, is_json=False)
    ...
    def export_prediction(self, name: str, format: str):
        ...
        """
        params = {"project_name": self.project_name, "name": name, "format": format}
        r = self._get("/export/prediction", params=params, is_json=False)
    ...
    def export_bert(self, name: str):
        ...
        if name is None:
            return None
        params = {"project_name": self.project_name, "name": name}
        r = self._get("/export/bert", params=params, is_json=False)
        if type(r) is dict:
            print(r)
            return None
    ...
    def compute_projection(self):
        ...
        r = self._post("/elements/projection/compute", params=params, json_data=data)
        if r["status"] == "waiting":
            self.projection_data = "computing"
            self.visualization.children = [
                widgets.HTML(
                    value="<div  style='background-color: #ffcc00; padding: 10px;'>Computing</div>"
                )
            ]
        else:
            print(r)
    ...
    def plot_visualisation(self):
        ...
        df["to_show"] = df.apply(
            lambda x: f"{x.name}<br>{'<br>'.join(textwrap.wrap(x['texts'][0:300],width=30))}...",
            axis=1,
        )
        f = go.FigureWidget(
            [
        ...
        def update_point(trace, points, selector):
            ...
            if len(points.point_inds) > 0:
                element_id = trace.customdata[points.point_inds][0][0].split("<br>")[
                    0
                ]  # TODO améliorer
    ...
    def get_projection_data(self):
        ...
        r = self._get("/elements/projection/current", params=params)
        return r
    ...
    async def update_state(self):
        ...
        while Widget.async_update:
            ...
            if self.bert_training and (
                not self.user in self.state["bertmodels"]["training"]
            ):
                self.bert_training = False
            ...
            if (type(self.projection_data) is str) and (
                self.projection_data == "computing"
            ):
            ...
            if self.computing_simplemodel and (
                not self.user in self.state["simplemodel"]["training"]
            ):
    ...
    def interface(self):
        ...
        # Tab schemes
        # ------------
        self.select_scheme = widgets.Dropdown(
            description="Select: ", value="", options=[""]
        )
        valid_delete_scheme = widgets.Button(
            description="Delete", button_style="danger"
        )
        valid_delete_scheme.on_click(
            lambda b: self.delete_scheme(self.select_scheme.value)
        )
        ...
        valid_delete_label.on_click(
            lambda b: self.delete_label(self.select_label.value)
        )
        ...
        delete_feature.on_click(
            lambda x: self.delete_feature(self.available_features.value)
        )
        self.add_features = widgets.Dropdown(
            description="Add: ", value="", options=[""]
        )
        self.features_params = widgets.Textarea(
        ...
        def if_change_feature(change):
            if change["type"] == "change" and change["name"] == "value":
                self.features_params.value = json.dumps(
                    self.state["features"]["options"][self.add_features.value], indent=2
                )

        self.add_features.observe(if_change_feature)
        valid_compute_features = widgets.Button(description="⚙️Compute")
        valid_compute_features.on_click(
            lambda x: self.compute_feature(
                self.add_features.value, self.features_params.value
            )
        )
        ...
        tab_features = widgets.VBox(
            [
                ...
                widgets.HTML(value="<hr>"),
                widgets.HBox(
                    [self.add_features, self.features_params, valid_compute_features]
                ),
                widgets.HTML(value="<hr>"),
        ...
        self.add_context = widgets.ToggleButton(
            description="Context", value=False, icon="valid"
        )
        ...
        def on_change_method(change):
            if change["type"] == "change" and change["name"] == "value":
                self.projection_params.value = json.dumps(
                    self.state["projections"]["available"][
                        self.projection_method.value
                    ],
                    indent=2,
                )

        self.projection_method.observe(on_change_method)
        self.projection_params = widgets.Textarea(
            layout=widgets.Layout(width="200px", height="100px")
        )
        ...
        self.projection = widgets.Accordion(
            children=[
                widgets.VBox(
                    [
                        widgets.HBox([self.projection_method, self.projection_compute]),
                        widgets.HBox(
                            [self.projection_features, self.projection_params]
                        ),
                        self.visualization,
                    ]
                )
        ...
        # Tab data
        # ---------
        self.sample_type = widgets.Dropdown(
            description="On: ",
            value="recent",
            options=["all", "tagged", "untagged", "recent"],
            layout={"width": "200px"},
        )
        self.sample_min = widgets.IntText(
            value=0, description="Min:", disabled=False, layout={"width": "200px"}
        )
        self.sample_max = widgets.IntText(
            value=0, description="Max:", disabled=False, layout={"width": "200px"}
        )
        ...
        def send_table():
            data = {
                "scheme": self.select_scheme.value,
                "list_ids": [
                    i.children[-1].layout.id for i in self.display_table.children
                ],
                "list_labels": [
                    i.children[-1].value for i in self.display_table.children
                ],
            }
            r = self._post(
                "/elements/table",
        ...
        tab_data = widgets.VBox(
            [
                widgets.HBox(
                    [self.sample_type, self.sample_min, self.sample_max, valid_sample]
                ),
                self.display_table,
                modify_table,
            ]
        ...
        # Tab statistics
        # ---------------
        self.data_description = widgets.HTML(
        ...
        def on_change_scheme(change):
            if change["type"] == "change" and change["name"] == "value":
                self.simplemodel_params.value = json.dumps(
                    self.state["simplemodel"]["options"][self.select_simplemodel.value],
                    indent=2,
                )

        self.select_simplemodel.observe(on_change_scheme)
        self.select_features = widgets.SelectMultiple()
        self.simplemodel_params = widgets.Textarea(
            value=json.dumps({}), layout=widgets.Layout(width="300px", height="200px")
        )
        valid_model = widgets.Button(description="⚙️Train")
        valid_model.on_click(
            lambda b: self.create_simplemodel(
                scheme=self.select_scheme.value,  # attention il faudra revoir le choix du scheme
                model=self.select_simplemodel.value,
                parameters=self.simplemodel_params.value,
                features=self.select_features.value,
            )
        )
        self.simplemodel_autotrain = widgets.IntSlider(min=5, max=50, description="")
        self.computing_simplemodel = False
        # Populate
        self.update_tab_simplemodel()
        ...
        tab_simplemodel = widgets.VBox(
            [
                ...
                widgets.HBox([self.select_features, self.simplemodel_params]),
                valid_model,
        ...
        def on_change_model(change):  # if select one, display its options on_select
            if (
                ...
                and self.available_bert.value is not None
            ):
                # available predict button
                if self.state["bertmodels"]["available"][self.select_scheme.value][
                    self.available_bert.value
                ]["predicted"]:
                    self.compute_prediction.disabled = True
                else:
        ...
        self.bert_summary = widgets.Accordion(
            children=[self.bert_statistics], titles=("Description",)
        )

        self.compute_prediction = widgets.Button(
            description="Compute prediction", disabled=True
        )
        self.compute_prediction.on_click(
            lambda x: self._start_bert_prediction(self.available_bert.value)
        )
        self.new_bert_base = widgets.Dropdown(description="Base:")
        self.new_bert_params = widgets.Textarea(
            layout={"width": "200px", "height": "200px"}
        )
        ...
        self.bert_name = widgets.Text(
            description="Name:", layout={"width": "150px"}, value="Name"
        )
        self.record_bert = widgets.Button(description="Save Bert")
        self.record_bert.on_click(
            lambda x: self.save_bert(self.available_bert.value, self.bert_name.value)
        )
        ...
        tab_bertmodel = widgets.VBox(
            [
                self.bert_status,
                widgets.HBox(
                    [self.available_bert, delete_bert, self.compute_prediction]
                ),
                self.bert_summary,
        ...
        # data
        export_tagged_data_presentation = widgets.HTML(
            value="<hr>Export tagged data<br>"
        )
        # export_tagged_data_columns = widgets.SelectMultiple(layout = layout_menu)
        export_tagged_data_format = widgets.Dropdown(
            options=["csv", "parquet"], layout=layout_button
        )
        valid_export_tagged_data = widgets.Button(description="⬇", layout=layout_button)
        valid_export_tagged_data.on_click(
            lambda x: self.export_data(format=export_tagged_data_format.value)
        )
        ...
        export_features_format = widgets.Dropdown(
            options=["csv", "parquet"], layout=layout_button
        )
        valid_export_features = widgets.Button(description="⬇", layout=layout_button)
        valid_export_features.on_click(
            lambda x: self.export_features(
                features=self.export_features_columns.value,
                format=export_features_format.value,
            )
        )

        # bert predictions
        export_predictions_presentation = widgets.HTML(
            value="<hr>Export BERT predictions<br>"
        )
        self.select_bert_model_predict = widgets.Dropdown(layout=layout_menu)
        export_bert_format = widgets.Dropdown(
            options=["csv", "parquet"], layout=layout_button
        )
        valid_export_predict = widgets.Button(description="⬇", layout=layout_button)
        valid_export_predict.on_click(
            lambda x: self.export_prediction(
                name=self.select_bert_model_predict.value,
                format=export_features_format.value,
            )
        )
        ...
        valid_export_bertmodel.on_click(
            lambda x: self.export_bert(name=self.select_bert_model.value)
        )

        tab_export = widgets.VBox(
            [
                export_tagged_data_presentation,
                widgets.HBox(
                    [
                ...
                export_predictions_presentation,
                widgets.HBox(
                    [
                ...
                export_bert_presentation,
                widgets.HBox([self.select_bert_model, valid_export_bertmodel]),
            ]
        )
